...
export default function Chatbot() {
  ...
  useEffect(() => {
    ...
    async function fetchProfile() {
      try {
        ...
        const resp = await fetch(`${import.meta.env.VITE_BACKEND_URL}/profile`, { headers, credentials: 'include' });
        if (!mounted) return;
        if (!resp.ok) return;
  ...
  const handleSend = (txt) => {
    ...
    (async () => {
      try {
        const resp = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/chatbot/respond`, {
          method: 'POST',
